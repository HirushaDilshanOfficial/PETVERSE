...
export async function getAllServices(req, res) {
  try {
    ...
    const normalized = services.map((doc) => {
      ...
      if ((obj.price === undefined || obj.price === null) && Array.isArray(obj.packages) && obj.packages.length) {
...
export async function createService(req, res) {
  try {
    ...
    console.log('Destructured data:', { title, description, address, category, packages });
    ...
    if (packages) {
      try {
        ...
        safePackages = typeof packages === 'string' ? JSON.parse(packages) : packages;
    ...
    const service = new Service({
      ...
      packages: safePackages,
      provider: {
        userId: req.user?._id || null, // use logged-in user ID from auth middleware
...
export async function updateService(req, res) {
  try {
    const { title, description, address, category, images, packages } = req.body;
